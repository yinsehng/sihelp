...
function actionClear(id){
	ymPrompt.confirmInfo({title:'信息提示',message:'确认是否执行操作？',handler:handler});
}
function handler(key){
	if(key=='ok'){
		window.open('<%=basePath%>servlet/UpdLocalDbServlet?t='+Math.random(),'_self');
	}
}
</script>
...
String lc=pro.getValue("localdbcount");
int lccount=Integer.parseInt(lc);
%>
<%if(mes!=null){%>
      ...
      <tr>
        <td height="80" bgcolor="#FFFFFF" class="STYLE6"  style="text-align: left;" colspan="2">
          <div style="width: 30%;float: left;padding:10px 0px 10px 30px;"> 
			 <img src="<%=basePath %>images/cc.gif" style="float: left;margin-left:20px;" border="0"/>
		   </div>
		   <div style="width: 30%;float: left; margin-top: 10px;">
		   	 <%if(lccount-count>0){%>
		   	    <a href="javascript:void(0)" onclick="actionClear()"><img src="<%=basePath %>images/updjg.gif" style="float: left;margin-top: 34px;" border="0"/></a>
		   	 <%}else{ %>
		   	    <img src="<%=basePath %>images/updjgo.gif" style="float: left;margin-top: 34px;" border="0"/>
		   	 <%} %>
		   ...
		   <div style="width: 30%;float: left; margin-top: 10px;"> 
		     ...
		     <div style="float: left;margin-left: 5px;width: 75%;font-size: 14px;line-height: 21px;">
		     	<strong>说明：</strong><br/><font color="black" >该功能用于修改数据传输时间间隔，如新增药品后传输数据慢时使用该功能，如果按钮为灰色则当天不能再使用该功能。</font>
		     </div>
		   </div>
		</td>
      </tr>
      ...
      <tr>
        ...
        <td width="10%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">已使用次数</span></div></td>
       		...
       		<tr onmouseover="moves(' ')" onmouseout="outs(' ')" id="t_ " >
		        ...
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center"><%=lccount-count %></div></td>
