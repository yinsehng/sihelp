...
int kc33count=Integer.parseInt(kc33);
%>
<%if(mes!=null){%>
      ...
      <tr>
        <td height="80" bgcolor="#FFFFFF" class="STYLE6"  style="text-align: left;" colspan="2">
          <div style="width: 30%;float: left;padding:10px 0px 10px 30px;"> 
			 <img src="<%=basePath%>images/err1.jpg" width="250px" height="100px"/>
		   </div>
		   <div style="width: 30%;float: left; margin-top: 10px;">
		   	 <%if(kc33count-count>0){%>
		   	    <a href="javascript:void(0)" onclick="actionClear()"><img src="<%=basePath %>images/clearbt.gif" style="float: left;margin-left:20px; margin-top: 30px;" border="0"/></a>
		   	 <%}else{ %>
		   	    <img src="<%=basePath %>images/clearbto.gif" style="float: left;margin-left:20px; margin-top: 30px;" border="0"/>
		   	 <%} %>
		   </div>
		   <div style="width: 30%;float: left; margin-top: 10px;"> 
		     ...
		     <div style="float: left;margin-left: 5px;width: 75%;font-size: 14px;line-height: 21px;">
		     	<strong>说明：</strong><br/><font color="black" >该功能用于非正常退出系统后无法登录系统提示超出用户数限制时使用,如出现左侧图片提示错误请点击清除登录按钮清除一个点,如果按钮为灰色则当天不能再使用该功能。</font>
		     </div>
		   </div>
		</td>
      </tr>
      ...
      <tr>
        ...
        <td width="10%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">已使用次数</span></div></td>
       		...
       		<tr onmouseover="moves(' ')" onmouseout="outs(' ')" id="t_ " >
		        ...
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center"><%=kc33count-count %></div></td>
