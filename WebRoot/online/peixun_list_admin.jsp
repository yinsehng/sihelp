...
$(function(){
	$("#del").click(function(){
		if($("[name='cid']:checked").length<1){
			ymPrompt.errorInfo({title:'错误提示：',message:'请选择要修改的记录!'});
		}else{ 
	    	ymPrompt.confirmInfo({message:'确定要修改吗？',title:'修改提示：',handler:upd});
	    }
	}); 
...
function upd(t){
    if(t=='ok'){//执行删除
		ymPrompt.win({message:' <center><strong>点击下方按钮进行状态修改</strong> <br/>已报名：修改状态为已报名<br/>已确认：修改状态为已确认<br/>已培训：修改状态为已培训',title:'修改培训状态</center>',handler:del,btn:[['已报名','b1'],['已确认','b2'],['已培训','b3'],['取消报名','b4']]});
    }
}
...
function del(t){
    ...
    if(t!='close'){
    	$("form").attr("action", "<%=basePath%>oper/UpdOperatorAdmin.do?ids="+str+"&flag="+zt);
    	$("form").submit();
    }
...
var delid;
function delonly(id){
	ymPrompt.win({message:'<br/> <center><strong>点击下方按钮进行状态修改</strong> <br/>已报名：修改状态为已报名<br/>已确认：修改状态为已确认<br/>已培训：修改状态为已培训',title:'修改培训状态</center>',handler:delonlyone,btn:[['已报名','b1'],['已确认','b2'],['已培训','b3'],['取消报名','b4']]});
	delid=id;
}
...
function delonlyone(t){
    ...
    if(t!='close'){
    	$("form").attr("action", "<%=basePath%>oper/UpdOperatorAdmin.do?ids="+delid+"&flag="+zt);
    	$("form").submit();
    }
}

function inserqxyy(id){
	var qxyy=$("#d"+id).val();
	qxyy=escape(escape(qxyy));
	$.ajax({
		url:"<%=basePath%>oper/UpdQxyy.do?id="+id+"&qxyy="+qxyy+"&t="+new Date(),
		timeout: 100000,
		dataType:"text",
		type:"post",
		success:function(t){
...
<body>
<form action="<%=basePath %>oper/SelOperatorAdmin.do" method="post" id="sform">
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
      ...
      <tr>
		...
		操作员培训报名列表
 		 ...
 		 <td bgcolor="#d3eaef" ><div align="right"><span class="STYLE1">
              ...
              全选      &nbsp;&nbsp; 
              <img src="<%=basePath %>images/del.gif" width="10" height="10" /> <a id="del" style="cursor: pointer;">批量修改</a>   &nbsp;  &nbsp;
              </span></div></td>
      </tr>
      <tr>
        <td height="80" bgcolor="#FFFFFF" class="STYLE6"  style="text-align: left;" colspan="2">
          <div style="width: 30%;float: left;padding:10px 0px 10px 30px;"> 
			<ul>
			  <li>定 点 编号：<input type="text" name="akb020" value="${param.akb020 }"/></li>
			  <li>定 点 名称：<input type="text" name="akb021" value="${param.akb021 }"/></li>
		   ...
		   <div style="width: 30%;float: left; margin-top: 10px;"> 
		     ...
		     <div style="float: left;margin-left: 5px;width: 75%">
		     	<strong>说明：</strong><font color="black">培训状态：1已报名:可以进行修改和删除，2报名确认：培训方已经确认安排培训不可以删除和修改，3已培训：已参加培训<br/></font>
		     </div>
		   </div>
		</td>
      </tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" >
     <tr>
        <td colspan="12"  height="35" bgcolor="#d3eaef" class="STYLE6" style="text-align: center;padding-left: 5px;font-weight: bold;color: #3B6375;">
		已参加人员列表
 		</td>
      ...
      <tr>
        ...
        <td width="5%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">定点编号</span></div></td>
        <td width="15%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">定点名称</span></div></td>
        ...
        <td width="10%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">联系电话</span></div></td>
         ...
         <td width="20%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">审核说明</span></div></td>
      ...
      <c:forEach items="${page.list}" var="p">
		    <tr onmouseover="moves('${p.id }')" onmouseout="outs('${p.id }')" id="t_${p.id }" >
		        ...
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${p.akb020}</div></td>
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${p.akb021}</div></td>
		        ...
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19">
		        	<div align="center">
		        		<input type="text" value="${p.qxyy }" id="d${p.id}" onblur="inserqxyy('${p.id}')"/> 
		        	</div>
		        </td>
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${p.aae036 }</div></td>
		        <td height="20" bgcolor="#FFFFFF" class="STYLE19">
		        	<div align="center">
				        <input type="button" value="查看信息" onclick="javascript:window.open('<%=basePath %>oper/SelOperForUpd.do?pid=${p.id }&type=admin', '_self')"/>&nbsp;&nbsp;
				    	<input type="button" value="修改状态" onclick="delonly('${p.id}')"/>
		        	</div>
		        </td>
		    </tr>
     	</c:forEach>	
